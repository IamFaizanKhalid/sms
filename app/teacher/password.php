...
if ( isset( $_POST[ "chpass" ] ) ) {
	...
	$npass = $_POST[ "newpass" ];
	if ( $opass != '' && $npass != '' ) {
		$rp = $conn->query( "SELECT passwd FROM login WHERE u_id='" . $_SESSION[ "u_id" ] . "' AND u_type=2 LIMIT 1" )->fetch_assoc();
		if ( $rp[ "passwd" ] == $opass ) {
			if ( strlen( $npass ) < 5 || strlen( $npass ) > 16 )
				$npErr = '* Password must be between 5-16 characters.';
			else {
				$conn->query( "UPDATE login SET passwd='" . $npass . "' WHERE u_id='" . $_SESSION[ "u_id" ] . "'  AND u_type=2" );
