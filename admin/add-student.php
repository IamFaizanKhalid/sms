...
$name = $uname = $uname_err = $rollno = $roll_err = $father_name = $dob = $bform = $father_cnic = $board_reg_no = $religion = $nationality = $blood_group = $phone1 = $phone2 = $address = $pic_err = '';
$name_err = $dob_err  = $gender_err = $cls_err = false;
$gender = 'X';
$married = -1;
$pic = '/images/student/default.jpg';
...
if ( isset( $_POST[ "save" ] ) ) {
	...
	if ($conn->query("SELECT name FROM student WHERE cls_id='$cls_id' AND rollno='$rollno'")->num_rows){
	...
	if ( $ok ) {
		...
		$conn->query( "INSERT INTO student(name, gender, dob, religion, nationality, blood_group, cnic, pic, address, phone1, phone2, rollno, father_name, father_cnic, board_reg_no, cls_id) VALUES('$name', '$gender', '$dob', '$religion', '$nationality', '$blood_group', '$bform', '$pic', '$address', '$phone1', '$phone2', '$rollno', '$father_name', '$father_cnic', '$board_reg_no', '$cls_id')" );
		$std_id = $conn->insert_id;
		$conn->query( "INSERT INTO login VALUES('$std_id', '1', '$uname', (SELECT value FROM controlVars WHERE  var='default_pass_s'), NULL)" );

		echo '<form id="myForm" method="post" action="students"><input type="hidden" name="cls" value="' . $cls_id . '"></form><script type="text/javascript">document.getElementById("myForm").submit();</script>';
		exit();
