...
$result = $conn->query( "SELECT r.total, r.obtained, s.sub_name FROM result r LEFT JOIN teacherAssigned x ON r.sub_dtl_id=x.id INNER JOIN subject s ON x.sub_id=s.sub_id WHERE r.std_id='" . $std_id . "' AND r.ex_id='" . $ex_id . "'" );

//$resultTotal = $conn->query( "SELECT * FROM resultTotal WHERE std_id='" . $std_id . "' AND ex_id='" . $ex_id . "' LIMIT 1" )->fetch_assoc();
$resultTotal = $conn->query( "SELECT SUM(total) total, SUM(obtained) obtained FROM result WHERE std_id='" . $std_id . "' AND ex_id='" . $ex_id . "'" )->fetch_assoc();
if ($resultTotal["total"])
$percentage = round(100*$resultTotal["obtained"]/$resultTotal["total"], 2 );
else
	$percentage=0;
if ($percentage>=80) $grade= 'A+'; else if ($percentage>=70) $grade= 'A'; else if ($percentage>=60) $grade= 'B'; else if ($percentage>=50) $grade= 'C'; else if ($percentage>=40) $grade= 'D'; else if ($percentage>=33) $grade= 'E'; else $grade= 'F';

$stdinfo = $conn -> query("SELECT s.name, s.rollno, c.cls_name, c.cls_section FROM student s INNER JOIN classes c ON s.cls_id=c.cls_id WHERE s.std_id=$std_id LIMIT 1")->fetch_assoc();
 ...
 ?>
		...
		<table>
			<tr>
				...
				<th>Exam</th>
				<td><?php  mysqli_data_seek($exams,0); while ($x = $exams->fetch_assoc()) if ($x["ex_id"] == $ex_id) { echo $x["ex_name"].' ('.$x["ex_year"].')'; break; } ?></td>
				</tr>
			<tr>
				<th>Class</th>
				<td><?php echo $stdinfo["cls_name"].' ('.$stdinfo["cls_section"].')'; ?></td>
				<th>Roll No.</th>
				<td><?php echo $stdinfo["rollno"]; ?></td>
			</tr>
			<tr>
				<td colspan="4">
	...
	if (!$x["total"] || 100*$x["obtained"]/$x["total"] < $passing_marks)
						echo '<tr>
